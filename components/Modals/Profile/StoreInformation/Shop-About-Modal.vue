<template>
  <TransitionGroup name="modal">
    <div
      v-if="activeModal == 'ShopAboutModal'"
    ...
    >
      ...
      >
        ...
        <div class="text-right inline-flex items-center pr-8 py-4 border-b w-full">
          <div class="info">
            <h3 class="text-xl font-semibold block text-gray-500"> درباره فروشگاه</h3>
          </div>
        </div>
        <div class="modal-body font-fa mx-2 px-6 my-4">
          ...
          <Button
            ...
            :isShow="about_store == null ? false : true"
          >
            <i class="fa-solid fa-edit pl-2 text-xl"></i> تایید
          </Button>
          <Button
            @click="change_about_store()"
...
const props = defineProps(["activeModal" , "aboutStore"]);
const emit = defineEmits(["close"]);
const about_store = ref(null)
...
onMounted(() => {
  if(props.aboutStore != null && props.aboutStore != ""){
     about_store.value = props.aboutStore
  }
})

const change_about_store = async () => {
  if (about_store != null) {
    loadingButton.value = true;
    const result = await sellerStore.change_about_store(about_store.value);
    ...
    toast.error("لطفا درباره فروشگاه توضیحاتی را وارد کنید");
  }
};
</script>
