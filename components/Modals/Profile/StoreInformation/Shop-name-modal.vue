<template>
    <TransitionGroup name="modal">
      <div
        v-if="activeModal == 'ShopNameModal'"
      ...
      >
        ...
        >
          ...
          <div class="text-right inline-flex items-center pr-8 py-4 border-b w-full">
            <div class="info">
              <h3 class="text-xl font-semibold block text-gray-500">ثبت نام فروشگاه</h3>
            </div>
          </div>
          <div class="modal-body font-fa mx-2 px-6 my-4">
            ...
            <Button
              ...
              :isShow="store_name == null ? false : true"
            >
              <i class="fa-solid fa-edit pl-2 text-xl"></i> تایید
            </Button>
            <Button
              @click="change_store_name()"
  ...
  const props = defineProps(["activeModal" , "storeName"]);
  ...
  onMounted(() => {
    if(props.storeName != null && props.storeName != ""){
        store_name.value = props.storeName
    }
  })
  
  const change_store_name = async () => {
    if (store_name != null) {
      loadingButton.value = true;
      const result = await sellerStore.change_store_name(store_name.value);
