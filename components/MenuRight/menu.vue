<template>
    <div class="relative w-full h-full inset-0 z-[100]">
        <!-- bg open -->
        <span class="fixed bg-gray-900 backdrop-blur-[2px] bg-opacity-30 w-full h-full inset-x-0 top-0 block lg:hidden" @click="NavMenu = !false" :class="NavMenu == false ? 'w-full translate-x-0 transition-opacity' : 'hidden'"></span>
        <nav :class="NavMenu == false ? 'translate-x-0' : 'translate-x-80'" class="p-3 fixed  w-64 transition-all duration-500 ease-in-out h-screen bg-white/90 dark:bg-gray-800/90 border-l z-[1000] right-0 top-0">
                ...
                <div class="mh-18 text-center py-5">
                    <a href="#" class="relative">
                        <h2 class="text-2xl font-semibold text-gray-200 px-4 max-h-9 overflow-hidden hidden-compact">
                            <img class="inline-block w-20 h-auto ltr:mr-2 rtl:ml-2 -mt-1" src="@/assets/images/petman.png">
                        </h2>
                    </a>
                </div>
                ...
                <ul class="flex flex-col gap-1">
                    ...
                    <li>
                        <button @click="openSubMenu(23)" class="middle none font-fa font-bold center transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-2 rounded-lg text-gray-500 active:opacity-[0.85] active:bg-green-50/20 focus:bg-gray-100 focus:text-gray-500 w-full flex justify-between items-center gap-4 px-4 capitalize"
                          ...
                          <i :class="submenuIndex == 23 ? 'fa-chevron-down' : 'fa-chevron-left'"
                            class="fa-light text-[15px] text-slate-500"></i>
                        </button>
                        <!-- Dropdown menu -->
                        <collapse-transition>
                            <div class="z-10 px-1 font-fa">
                                <ul class="text-sm text-gray-700 dark:text-gray-400" aria-labelledby="dropdownNavbarButton">
                                    <li>
                                        <RouterLink to="/Ecommerce/Products/AddProduct">
                                            <span class="flex items-center justify-between w-full gap-4 px-4 py-2 text-xs font-bold text-gray-500 capitalize transition-all rounded-lg middle none font-fa center disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none bg-gradient-to-r">
                                    <span class="pr-1">
                                      <i class="fa-light fa-draw-circle text-[12px] text-gray-500"></i>
                                    </span>
                                            <p class="block w-full text-base antialiased font-medium leading-relaxed text-right capitalize font-fa">
                                                اضافه کردن محصول
                                            </p>
                    ...
                    <li>
                        <nuxt-link to="/withdraw/">
                            <button class="middle none font-fa font-bold center transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-2 rounded-lg text-gray-500 active:opacity-[0.85] active:bg-green-50/20 focus:bg-gray-100 focus:text-gray-500 w-full flex justify-between items-center gap-4 px-4 capitalize"
                                type="button">
                          <span class="pr-1">
                            <i class="fa-light fa-money-bill-transfer text-[18px] text-slate-500"></i>
                          </span>
                          <p class="block w-full text-base antialiased font-medium leading-relaxed text-right capitalize font-fa">
                            تسویه حساب
                          </p>
